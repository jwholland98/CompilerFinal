...
bool exists(Token &next) { // Checks if var is already declared
...
class Parser {
							...
							if(next.type==CLOSE_BRACKET) {
								...
								if(next.type==ELSE) {
									...
									if(next.type==OPEN_BRACKET) {
										...
										SymbolTable.push_back(s);
										next=tokenizer.peek();
										while(statement(next)) { next=tokenizer.next(); }
										if(next.type==CLOSE_BRACKET) {
											ifelsej = next.value + "\n";
											s.statement = ifelsej;
											SymbolTable.push_back(s);
	...
	bool forloop(Token &next) {
		string forj; // string to store for loop
		StateTree s;
		next=tokenizer.next();
				...
				if(declaration(next)) {
					...
					if(expression(*subtree)){ // Possibly too vague
						...
						if(next.type==SEMICOLON) {
							...
							SymbolTable.push_back(s);
							next=tokenizer.peek();
							while(statement(next)) { // Doesn't allow for i++
								next=tokenizer.next();
								if(next.type==CLOSE_PAREN) {
									forj = next.value;
									next=tokenizer.next();
	...
	bool whileloop(Token &next) {
		...
		if(next.type==WHILELOOP) {
			...
			if(next.type==OPEN_PAREN) {
				whilej+=next.value;
				ExpressionTree *subtree = new ExpressionTree;
				if(expression(*subtree)){ // Doesn't put in while
	...
	bool output(Token &next) {
		...
		if(next.type==COUT) {
			...
			while(next.type==OSTREAM) {
				...
				if(next.type==QUOTES) {
					outj += "\"";
					next=tokenizer.next(); // Possible issue w/ logic, doesn't always grab everything between quotes
					outj += next.value;
					next=tokenizer.next();
					if(next.type==QUOTES) {
						outj+="\"";
				...
				else if(next.type==ENDL) { // Checks if var is already declared
					outj+="\"\\n\"";
					next=tokenizer.peek();
				}
			}
    ...
    bool expression(ExpressionTree &tree) {
		...
		while(additiveExpression(*subtree)) {
			...
			if(!(next.type==RELATIONAL_OP)) {
				if(last.type==EOL) // For case where there is no relational op in the overall expression
					tree=*subtree;
				break;
			}
			else {
    ...
    bool additiveExpression(ExpressionTree &tree) {
		...
		while(multiplicativeExpression(*subtree)) {
			...
			if(!(next.type==ADDITIVE_OP || next.type==PLUSMINUS_OP)) {
				if(last.type==EOL){ // For case where there is no additive op in the overall expression
					tree=*subtree;
				}
				if(next.type==RELATIONAL_OP)
					flag=true;
    ...
    bool multiplicativeExpression(ExpressionTree &tree) {
		...
		while(unaryExpression(*subtree)) {
			...
			if(!(next.type==MULTIPLICATIVE_OP)) {
				if(last.type==EOL) // For case where there is no multiplicative op in the overall expression
					tree=*subtree;
