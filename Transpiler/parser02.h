...
class Parser{
	...
	bool statement(Token &next){
		...
		cout << "STATEMENT: value 'next' is: " << next.value << endl;
		if(next.type==DATATYPE){
			...
			cout << "Heading into while loop..." << endl; // ++++++++++++++++++++++++++++++++++++++++++++++++++++++
			if(whileloop(next)){
				cout << "Made it" << endl;
				return true;
			}
			...
			cout << "STATEMENT: in VARNAME. 'next' is: " << next.value << endl;
			next=tokenizer.next();
						...
						if(next.type==SEMICOLON){
							...
							while(statement(next)){//doesnt allow i++
								...
								if(next.type==CLOSE_PAREN){
									forj = next.value;
									next=tokenizer.next();
									if(next.type==OPEN_BRACKET){ //++++++++++++++++++++++++++++++++++++++++++++++++++++ breaks in here
										forj += next.value+'\n';
										s.statement=forj;
										SymbolTable.push_back(s);
										next=tokenizer.peek();
										while(statement(next)){
											next=tokenizer.next();
											cout << "FORLOOP next.value is: " << next.value << endl;
										}
										if(next.type==CLOSE_BRACKET){
											forj =next.value + '\n';
											s.statement=forj;
    ...
    bool init_decl(ExpressionTree &tree){
        ...
        if(next.type==SEMICOLON){
            ...
            error.push_back("In init_decl: expected expression");
            return false;
        }
        error.push_back("In init_decl: expected equal or semicolon at end of expression");
        return false;
    }
